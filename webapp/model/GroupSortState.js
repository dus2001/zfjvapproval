sap.ui.define([
	...
	return BaseObject.extend("com.model.GroupSortState", {
		 ...
		 * @alias com.model.GroupSortState
		 */
		...
		/**
		 * Groups by Totamt, or resets the grouping for the key "None"
		 *
		 * @param {string} sKey - the key of the field used for grouping
		 * @returns {sap.ui.model.Sorter[]} an array of sorters
		 */
		group: function (sKey) {
			...
			if (sKey === "Totamt") {
				...
				this._oViewModel.setProperty("/sortBy", "Totamt");

				aSorters.push(
					new Sorter("Totamt", false,
						this._fnGroupFunction.bind(this))
				);
			} else if (sKey === "None") {
				// select the default sorting again
				this._oViewModel.setProperty("/sortBy", "WiText");
			}
